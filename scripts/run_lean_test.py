"""Lean Test: strip dead weight, verify transfer holds.

Removed:
1. Wake homeostatic scaling (hom.update) — redundant with sleep homeostasis
2. Intrinsic plasticity (ip.update) — untested, possibly noise
3. L2->VIP context gating edges — tested, zero effect
4. Weight decay from Oja (0.013*w) — double-decaying with sleep 0.95x

Same: true silence, sensory surface, temporal Oja (simplified), bio rates,
hierarchy, hippocampus, full sleep, STP, structural plasticity.

Two foundational changes from definitive test:
...
WEIGHT_DECAY = 0.0  # removed — sleep homeostasis (0.95x) handles weight regulation
...
def apply_learning(graph, lr_scale=1.0, is_replay=False, driving_replay_count=None):
    ...
    for et in PLASTIC_EDGE_TYPES:
        ...
        if et == EdgeType.DRIVING:
            ...
            if is_replay:
                lr = 0.001 * lr_scale
                dw = lr * (store.pre_trace * dst - dst * dst * store.weight)
                ...
                dw = effective_lr * pe_gate * (store.pre_trace * dst - dst * dst * store.weight)
        elif et == EdgeType.MODULATORY:
            dw = (0.001 * lr_scale) * (src * dst - dst * dst * store.weight)
        elif et == EdgeType.DISINHIBITION:
            dw = (0.002 * lr_scale) * (src * dst - dst * dst * store.weight)
        elif et in (EdgeType.INHIB_PERISOMATIC, EdgeType.INHIB_DENDRITIC):
            dw = (0.0001 * lr_scale) * (src * dst - dst * dst * store.weight)
        else:
            dw = (0.001 * lr_scale) * (src * dst - dst * dst * store.weight)
        store.weight += dw
...
def measure_apical(pred_name, target_name, symbols, graph, ns, mp, device,
    ...
    for s in range(steps):
        ...
        # hom + ip removed (sleep homeostasis handles weight regulation)
...
def measure_discrimination(symbols, seq, graph, ns, mp, device,
    ...
    for i in range(len(seq) - 1):
        ...
        ap_correct = measure_apical(pred, target, symbols, graph, ns, mp, device,
                                     theta, stp, tau_mult, steps=steps)
...
def main():
    ...
    for epoch in range(N_EPOCHS):
        ...
        if (epoch + 1) % MEASURE_EVERY == 0:
            ...
            d_acc, d_disc = measure_discrimination(symbols, day_names, graph, ns, mp, device,
                                                    theta, stp, tau_mult, steps=pd_steps)
    ...
    novel_acc_before, novel_disc_before = measure_discrimination(
        symbols, novel_seq, graph, ns, mp, device, theta, stp, tau_mult)
