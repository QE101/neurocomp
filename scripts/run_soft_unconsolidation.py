"""Soft Un-consolidation v3: rate-matched + stiffness² for stable equilibrium.

Previous un-consolidation attempts:
  v1 (hard threshold 0.3, rate 0.01): binary switch → limit cycle (0.088 ↔ 0.357)
  v2 (sigmoid 1.5, rate 0.001): still 15-150x faster than consolidation build → limit cycle

Root cause: RATE MISMATCH.
  Build rate: 0.0001 * co_act ≈ 0.00001/step (co_act product of two outputs, ~0.01-0.1)
  v2 un-consolidation: 0.001 * sigmoid * 0.3 ≈ 0.00015/step → 15x stronger than build
  → un-consolidation dominates whenever active → drives stiffness down → shuts off
  → build recovers → cycle repeats

v3 fix:
  1. Rate 0.0001 (was 0.001) — same order as consolidation build
  2. Center 2.0 (was 1.5) — only genuinely surprising errors, not average ones
  3. Slope 2.0 (was 3.0) — gentler transition across error range
  4. stiffness² (was stiffness) — creates STABLE equilibrium:
     - At low stiffness (0.1): amount ∝ 0.01 (negligible, build dominates)
     - At high stiffness (0.5): amount ∝ 0.25 (moderate, matches build)
     Equilibrium: stiffness ≈ √(co_act / sigmoid_val) ≈ 0.3-0.4
     Perturbations push BACK toward equilibrium (negative feedback).

Also tracks stiffness distribution (p10/p50/p90) to verify no limit cycle.

Based on run_level2_readout.py. All other parameters identical for clean comparison.
"""
...
PAUSE = 20
CHECKPOINT_DIR = Path('checkpoints/soft_unconsolidation')
...
# ================================================================
# LEARNING WITH SOFT UN-CONSOLIDATION v3
# ================================================================
def apply_memory_learning(graph, lr_scale=1.0, is_replay=False):
    """Learning rule with rate-matched un-consolidation v3.

    v3 un-consolidation fixes the limit cycle from v1/v2:
    - Rate 0.0001 (matched to build rate, was 0.001)
    - Sigmoid center 2.0 (only genuinely surprising, was 1.5)
    - Sigmoid slope 2.0 (gentler transition, was 3.0)
    - stiffness² (nonlinear feedback → stable equilibrium, was linear)

    Equilibrium math:
      build = co_act * 0.0001 ≈ 0.00001/step
      unconsolidate = 0.0001 * sigmoid(err-2, slope=2) * stiffness²
      At equilibrium: stiffness ≈ √(co_act / sigmoid) ≈ 0.3-0.4
      This is CONVERGENT: perturbations push back toward equilibrium.
    """
    ...
    for et in PLASTIC_EDGE_TYPES:
        ...
        # === UN-CONSOLIDATION v3: rate-matched + stiffness² ===
        # Rate 0.0001 (same order as build rate 0.0001*co_act)
        # Center 2.0: only un-consolidate when error >> global average
        # Slope 2.0: gradual activation across error range
        # stiffness²: nonlinear → stable equilibrium, no limit cycle
        #   At stiff=0.1: 0.01 → negligible (build dominates, stiffness rises)
        #   At stiff=0.5: 0.25 → moderate (matched to build, equilibrium)
        #   At stiff=0.9: 0.81 → strong (but still needs high error to trigger)
...
def get_stiffness_stats(graph):
    """Get stiffness distribution across all plastic edges."""
    all_stiff = []
    for et in PLASTIC_EDGE_TYPES:
        if graph.has_edge_type(et):
            store = graph.edge_store(et)
...
def main():
    print('=' * 60, flush=True)
    print('  SOFT UN-CONSOLIDATION v3 TEST', flush=True)
    print('  Rate-matched (0.0001) + stiffness^2 + sigmoid(2.0, 2.0)', flush=True)
    print('  Fix: v2 limit cycle from 15x rate mismatch', flush=True)
    ...
    if latest_ckpt is not None:
        ...
        # Ensure new log keys exist for resumed runs
        for key in ['stiff_p10', 'stiff_p50', 'stiff_p90']:
            if key not in log:
    ...
    for epoch in range(start_epoch, N_EPOCHS):
        ...
        if (epoch + 1) % MEASURE_EVERY == 0:
            ...
            print(f'    echo={hl}st stiff={mean_s:.4f} [p10={p10:.4f} p50={p50:.4f} p90={p90:.4f}] edges={graph.n_edges():,} ({elapsed:.0f}s)', flush=True)
    ...
    print('  (Does soft un-consolidation let novel sequences break through?)', flush=True)
    ...
    for ep in range(200):
        ...
        if (ep + 1) % 50 == 0:
            ...
            _, l1_nd, _, l2_nd = measure_discrimination_dual(
                symbols, novel_seq, l2_reps, graph, ns, mp, device, theta, stp, hom, ip, tau_mult)
            mean_s, p10, p50, p90 = get_stiffness_stats(graph)
            print(f'  Novel ep{ep+1}: L1={l1_nd:+.1f}% L2={l2_nd:+.1f}% stiff={mean_s:.4f} [{p10:.4f}/{p50:.4f}/{p90:.4f}]', flush=True)
    ...
    # Results
    print(f'\n{"="*60}', flush=True)
    print('  RESULTS: Soft Un-consolidation v3', flush=True)
    print(f'  (rate=0.0001, center=2.0, slope=2.0, stiffness^2)', flush=True)
    ...
    if log['avg_stiffness']:
        ...
        print(f'    p10:  {log["stiff_p10"][0]:.4f} -> {log["stiff_p10"][-1]:.4f}', flush=True)
        print(f'    p50:  {log["stiff_p50"][0]:.4f} -> {log["stiff_p50"][-1]:.4f}', flush=True)
        print(f'    p90:  {log["stiff_p90"][0]:.4f} -> {log["stiff_p90"][-1]:.4f}', flush=True)
        # Check for limit cycle: look at variance across last 5 measurements
        if len(log['avg_stiffness']) >= 5:
            last5 = log['avg_stiffness'][-5:]
            stiff_range = max(last5) - min(last5)
            print(f'    last-5 range: {stiff_range:.4f} (< 0.05 = stable, > 0.10 = limit cycle)', flush=True)

    print(f'\n  Transfer (L1): {l1_final:+.1f}%', flush=True)
    print(f'  Transfer (L2): {l2_novel_before_disc:+.1f}% -> {l2_final:+.1f}% ({l2_transfer:+.1f}%)', flush=True)
    print(f'\n  KEY: stiffness range < 0.05 = no limit cycle (v2 was 0.27)', flush=True)
    print(f'  KEY: transfer > 0% = un-consolidation allows novel learning', flush=True)
    print(f'  Compare v2: transfer was -12.1%', flush=True)

    torch.save(log, 'soft_unconsolidation_results.pt')
