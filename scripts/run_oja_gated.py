"""Oja's Rule + Pre-synaptic Activity Gate: Combined Prevention.

Two mechanisms addressing two failure modes:
1. Activity gate: freezes unshared edges when their pattern isn't active
2. Oja's rule: self-normalizes shared edges (no explicit weight decay)

Oja's: dw = lr * post * (pre - weight * post)
- Self-normalizing: weight converges to pre/post ratio
- No separate decay term (the -weight*post^2 IS the decay, but proportional to activity)
- Shared edges find equilibrium between patterns instead of oscillating

Gate: only apply learning when source > threshold (0.85, above baseline 0.693)
- A's edges frozen during B's presentation (no cross-pattern decay)
- B's edges frozen during A's presentation

Combined: shared edges self-normalize via Oja, unshared edges protected by gate.
...
# ================================================================
# OJA'S RULE + PRE-SYNAPTIC ACTIVITY GATE
# ================================================================
ACTIVITY_THRESHOLD = 0.85

def apply_oja_gated(graph, la):
    """Oja's rule with pre-synaptic activity gate.

    Oja's: dw = lr * post * (pre - weight * post)
    - Self-normalizing (no explicit weight decay needed)
    - The -weight*post^2 term provides activity-proportional decay
    - Shared edges converge to equilibrium, not oscillation

    Gate: only update when source > threshold
    - Unshared edges frozen when their pattern isn't active
    """
    ...
    for et in EdgeType:
        if graph.has_edge_type(et) and et != EdgeType.ELECTRICAL:
            ...
            oja = dst * (src - store.weight * dst)

            # Activity penalty (keeps overall activity bounded)
            activity_penalty = la * (src + dst) * store.weight

            dw = 0.001 * (oja - activity_penalty) * gate
            store.weight += dw
...
def main():
    print('=' * 60, flush=True)
    print('  OJA + ACTIVITY GATE: Combined Prevention', flush=True)
    print('  Oja self-normalizes shared edges.', flush=True)
    print('  Gate freezes unshared edges during other pattern.', flush=True)
    print(f'  Threshold: {ACTIVITY_THRESHOLD} (baseline ~0.693)', flush=True)
    ...
    for cycle in range(N_CYCLES):
        ...
        for pat in [pa, pb]:
            for s in range(PD):
                ...
                apply_oja_gated(graph, LAMBDA_ACT)
        ...
        if (cycle + 1) % 500 == 0:
            ...
            # Measure gate stats
            total_gated, total_active = 0, 0
            for et in EdgeType:
                if graph.has_edge_type(et) and et != EdgeType.ELECTRICAL:
                    store = graph.edge_store(et)
                    if store.n_edges == 0: continue
                    src_act = ns.output[store.src.long()]
                    active = (src_act > ACTIVITY_THRESHOLD).sum().item()
                    total_active += active
                    total_gated += store.n_edges - active
            pct_frozen = total_gated / max(total_active + total_gated, 1) * 100

            # Measure weight stats
            w_mean = 0.0
            w_count = 0
            for et in EdgeType:
                if graph.has_edge_type(et) and et != EdgeType.ELECTRICAL:
                    store = graph.edge_store(et)
                    if store.n_edges == 0: continue
                    w_mean += store.weight.mean().item()
                    w_count += 1
            w_mean /= max(w_count, 1)

            mm = f'{ratio:.3f}x' + (' **' if ratio > 1.1 else '')
            print(f'  Cyc {cycle+1:5d}: mm={mm} sup={sup:.1f}% frozen={pct_frozen:.1f}% w={w_mean:.4f} ({elapsed:.0f}s)', flush=True)
