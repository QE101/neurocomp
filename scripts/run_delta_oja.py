"""Delta-Oja: Oja's rule on deviations from baseline.

Mathematical derivation:
- Instability requires dw != 0 at baseline (cross-pattern interference)
- All previous rules have nonzero dw at baseline:
  - Standard Hebbian: weight decay term fires always
  - Pure Oja: pushes all weights toward 1.0 at baseline (pre/post = 1)

- Fix: define delta = output - baseline, where baseline = softplus(0) = ln(2)
- dw = lr * delta_post * (delta_pre - w * delta_post)
- At baseline: delta = 0 -> dw = 0 exactly. Zero cross-pattern interference.
- At elevated activity: delta >> 0 -> standard Oja with O(delta^2) stabilization.
- Eigenvalue: 1 - lr*PD*delta_post^2 < 1 for all theta amplitudes.
...
PD = 50
N_CYCLES = 5000
BASELINE = math.log(2)  # softplus(0) = ln(2) = 0.6931...
...
# ================================================================
# DELTA-OJA: Oja's on deviations from baseline
# dw = lr * d_post * (d_pre - w * d_post)   where d = output - baseline
# ================================================================
def apply_delta_oja(graph):
    ...
    for et in EdgeType:
        if graph.has_edge_type(et) and et != EdgeType.ELECTRICAL:
            ...
            # Deviation from baseline (exactly 0 at resting state)
            d_pre = (pre - BASELINE).clamp(min=0.0)   # only positive deviations
            d_post = (post - BASELINE).clamp(min=0.0)

            # Oja's on deviations: O(d^2) potentiation, O(d^2) stabilization
...
def main():
    print('=' * 60, flush=True)
    print('  DELTA-OJA: Deviations from Baseline', flush=True)
    print(f'  dw = lr * d_post * (d_pre - w * d_post)', flush=True)
    print(f'  d = max(output - {BASELINE:.4f}, 0)', flush=True)
    print(f'  At baseline: d=0, dw=0. Zero cross-pattern interference.', flush=True)
    ...
    # Check initial activity distribution
    print(f'  Patterns: A={pa.shape[0]} nodes, B={pb.shape[0]} nodes', flush=True)
    print(f'  Total edges: {graph.n_edges():,}', flush=True)
    print(f'  Initial output mean: {ns.output.mean():.4f} (baseline={BASELINE:.4f})', flush=True)
    ...
    for cycle in range(N_CYCLES):
        ...
        if (cycle + 1) % 500 == 0:
            ...
            # Delta stats: what fraction of edges have nonzero delta?
            total_nonzero = 0
            total_edges = 0
            for et in EdgeType:
                if graph.has_edge_type(et) and et != EdgeType.ELECTRICAL:
                    ...
                    active = ((d_pre > 0.01) | (d_post > 0.01)).sum().item()
                    total_nonzero += active
                    total_edges += store.n_edges
            pct_active = total_nonzero / max(total_edges, 1) * 100

            mm = f'{ratio:.3f}x' + (' **' if ratio > 1.1 else '')
            print(f'  Cyc {cycle+1:5d}: mm={mm} sup={sup:.1f}% w={w_mean:.4f} active={pct_active:.1f}% ({elapsed:.0f}s)', flush=True)
    ...
    if best > 1.1 and sr < 0.3:
        ...
        print(f'\n  VERDICT: No mismatch (delta too weak for learning)', flush=True)
