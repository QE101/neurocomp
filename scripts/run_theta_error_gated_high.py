...
def apply_hebbian_error_gated(graph, la):
    ...
    for et in EdgeType:
        if graph.has_edge_type(et) and et != EdgeType.ELECTRICAL:
            ...
            # Error gate: only update if destination node has high error
            # dst output IS the error in the universal error model
            gate = torch.sigmoid(5.0 * (dst - ERROR_THRESHOLD))
            # dst >> threshold → gate ≈ 1.0 (high error, needs learning)
            # dst << threshold → gate ≈ 0.0 (low error, well-predicted, protect)
...
print(f'Theta=6Hz amp=0.5 | Error threshold={ERROR_THRESHOLD}', flush=True)
print(f'Only edges with dst error > {ERROR_THRESHOLD} get updated', flush=True)
...
for cycle in range(5000):
    ...
    if (cycle + 1) % 250 == 0:
        ...
        gate_mn = torch.sigmoid(5.0 * (ns.output[exc_idx] - ERROR_THRESHOLD)).mean().item()
