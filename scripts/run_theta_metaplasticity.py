...
consolidation = {}  # per-edge-type: how long each edge has been stable
CONSOL_GROW = 0.001   # consolidation grows this much per step when weight is stable
CONSOL_DECAY = 0.1    # consolidation drops this much per step when weight changes

def apply_hebbian_metaplastic(graph, la):
    """Hebbian with metaplasticity (synaptic consolidation).

    Each edge tracks how long it has been stable. Stable edges consolidate
    and become resistant to change. Recently changed edges are labile.

    consolidation grows when |dw| is small → edge is stable → resist change
    consolidation drops when |dw| is large → edge is labile → allow change

    effective_lr = base_lr / (consolidation + 1.0)

    Produces -e^(-x) + c learning curve:
    - New edges: consolidation ≈ 0, full learning rate → steep initial learning
    - Old stable edges: consolidation high, frozen → asymptotic plateau
    - Oscillation impossible: consolidated edges resist the sparsity penalty
    """
    ...
    for et in EdgeType:
        if graph.has_edge_type(et) and et != EdgeType.ELECTRICAL:
            ...
            if et not in consolidation:
                consolidation[et] = torch.zeros_like(store.weight)
            ...
            # Metaplastic learning rate: consolidated edges resist change
            consol = consolidation[et]
            effective_lr = 1.0 / (consol + 1.0)
            # consol=0 → lr=1.0 (fully labile, new edge)
            # consol=1 → lr=0.5 (half resistant)
            # consol=10 → lr=0.09 (90% resistant)
            # consol=100 → lr=0.01 (99% resistant, effectively frozen)

            dw = raw_dw * effective_lr
            store.weight += dw
            store.weight.clamp_(0.0, 1.0)

            # Update consolidation:
            # Grows when weight is stable (|dw| small), decays when weight changes (|dw| large)
            change_magnitude = raw_dw.abs()
            # Stable = change_magnitude < threshold (relative to weight magnitude)
            stability = torch.exp(-change_magnitude * 1000.0)  # ~1 when |dw|≈0, ~0 when |dw|>0.005
            consol_delta = CONSOL_GROW * stability - CONSOL_DECAY * (1.0 - stability)
            consolidation[et] = (consol + consol_delta).clamp(min=0.0)
...
def run_step(pat):
    ...
    apply_hebbian_metaplastic(graph, LAMBDA_ACT)
    if step % 100 == 0:
        for et in EdgeType:
            if graph.has_edge_type(et) and et != EdgeType.ELECTRICAL:
...
print('Theta + Metaplasticity: 5000 cycles', flush=True)
print('Theta=6Hz amp=0.5 | Synaptic consolidation: stable edges resist change', flush=True)
print(f'{"Cyc":>5} | {"Err":>7} | {"Sup%":>5} | {"Ap_std":>7} | {"Consol":>7} | {"EffLR":>6} | {"MM":>8} | {"Time":>5}', flush=True)
print('-' * 70, flush=True)
...
for cycle in range(5000):
    ...
    if (cycle + 1) % 250 == 0:
        ...
        # Mean consolidation across all edge types
        con_mn = np.mean([c.mean().item() for c in consolidation.values()]) if consolidation else 0
        con_mx = max([c.max().item() for c in consolidation.values()]) if consolidation else 0
        eff_lr_mn = 1.0 / (con_mn + 1.0)
        elapsed = time.perf_counter() - t0
        mm = f'{ratio:.3f}x' + (' **' if ratio > 1.1 else '')
        print(f'{cycle+1:5d} | {errors[-1]:.4f} | {sup:.1f}% | {ap:.4f} | {con_mn:.4f} | '
              f'{eff_lr_mn:.4f} | {mm:>8} | {elapsed:.0f}s', flush=True)

print(f'\nBest mismatch: {max(all_mm):.3f}x', flush=True)
print(f'Final mismatch: {all_mm[-1]:.3f}x', flush=True)
con_final = np.mean([c.mean().item() for c in consolidation.values()]) if consolidation else 0
con_max = max([c.max().item() for c in consolidation.values()]) if consolidation else 0
print(f'Consolidation: mean={con_final:.4f} max={con_max:.4f} (eff_lr={1.0/(con_final+1.0):.4f})', flush=True)
