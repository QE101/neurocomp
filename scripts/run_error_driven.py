"""Error-Driven Learning: the mathematically correct prevention.

Hebbian (pre * post) tracks correlations -> oscillates under alternating patterns.
Error-driven (pre * pred_err) tracks prediction error -> converges to zero.

When pattern A is learned: pred_err_A ~ 0 -> dw ~ 0. Self-limiting.
When pattern B is learned: pred_err_B ~ 0 -> dw ~ 0. Self-limiting.
Both learned -> update per cycle ~ 0 -> fixed point. No oscillation.

For modulatory edges (carry predictions to apical):
  dw = lr * pred_err_dst * content_src
  Reduces error by strengthening correct predictions.

For driving edges (carry errors via basal):
  dw = lr * pred_err_dst * output_src
  Reduces error by adjusting basal pathway weights.
...
# ================================================================
def apply_error_driven(graph):
    """Error-driven weight update using prediction error.

    Modulatory edges (apical predictions):
      dw = lr * pred_err_dst * content_src
      Strengthens prediction pathways that reduce error.

    Driving edges (basal input):
      dw = lr * pred_err_dst * output_src
      Adjusts input pathways based on error.

    Self-limiting: when pred_err -> 0, dw -> 0. No overshoot possible.
    """
    ns_ = graph.node_state
    pred_err = ns_.prediction_error  # basal - apical, per node
    output = ns_.output
    content = F.softplus(ns_.basal)
    ...
    for et in EdgeType:
        ...
        dst_err = pred_err[store.dst.long()]

        if et == EdgeType.MODULATORY:
            # Modulatory edges carry predictions to apical.
            # Positive pred_err = basal > apical = under-predicting.
            # Strengthen modulatory edges to increase apical (reduce error).
            src_signal = content[store.src.long()]
            dw = 0.001 * dst_err * src_signal
        elif et == EdgeType.DRIVING:
            # Driving edges carry input to basal.
            # Learning here adjusts how basal input is weighted.
            src_signal = output[store.src.long()]
            dw = 0.001 * dst_err * src_signal
        elif et == EdgeType.INHIB_PERISOMATIC:
            # PV inhibition: error-driven adjustment of inhibitory gain
            src_signal = output[store.src.long()]
            dw = -0.0005 * dst_err * src_signal  # negative: more error -> less inhibition
        elif et == EdgeType.INHIB_DENDRITIC:
            # SST inhibition of apical: error-driven
            src_signal = output[store.src.long()]
            dw = -0.0005 * dst_err * src_signal
        elif et == EdgeType.RETROGRADE:
            # Retrograde: feedback modulation
            src_signal = output[store.src.long()]
            dw = 0.0005 * dst_err * src_signal
        else:
            continue
...
def main():
    print('=' * 60, flush=True)
    print('  ERROR-DRIVEN LEARNING: Self-Limiting Prevention', flush=True)
    print('  dw = lr * pred_err * pre_signal', flush=True)
    print('  Learned patterns: err->0, dw->0. Fixed point.', flush=True)
    ...
    for cycle in range(N_CYCLES):
        ...
        if (cycle + 1) % 500 == 0:
            ...
            exc_mask = ns.type_mask(NodeType.EXCITATORY)
            pe_mag = ns.prediction_error[exc_mask].abs().mean().item()
            ...
            print(f'  Cyc {cycle+1:5d}: mm={mm} sup={sup:.1f}% pe={pe_mag:.4f} w={w_mean:.4f} ({elapsed:.0f}s)', flush=True)
    ...
    if len(all_mm) >= 4:
        ...
        if max_diff < 0.2 and best > 1.1:
            print(f'\n  VERDICT: FLAT TRAJECTORY -- oscillation PREVENTED', flush=True)
        elif best > 1.1 and sr < 0.3:
            print(f'\n  VERDICT: STABLE MISMATCH -- converged', flush=True)
        elif best < 1.05:
            print(f'\n  VERDICT: No mismatch (error-driven too weak)', flush=True)
