"""Hard Weight Normalisation v2: CEILING constraint, not equality.

v1 failed: normalising in both directions killed Hebbian differentiation.
Decay weakens unused edges (good), but normalisation scaled them back up (bad).

v2: Only normalise DOWN when budget is exceeded. Never scale UP.
This allows decay to create pattern-specific weight profiles while
preventing the overshoot that causes oscillation.

Budget = ceiling, not target.
...
# ================================================================
# v2: CEILING normalisation — only scale down, never up
# ================================================================
def compute_weight_budgets_fast(graph):
    """Capture initial per-node weight sum as the CEILING."""
    budgets = {}
    N = graph.n_nodes
    for et in EdgeType:
...
def normalise_weights_ceiling(graph, budgets):
    """Ceiling normalisation: only scale DOWN when total exceeds budget.
    If total is below budget, leave it alone — let decay do its work.

    This allows:
    - Hebbian potentiation up to the budget (redistribution within ceiling)
    - Hebbian decay to weaken unused edges (differentiation)
    - Prevents total from exceeding ceiling (no overshoot = no oscillation)
    """
    for et, budget in budgets.items():
        ...
        # Only scale down nodes that EXCEED their budget
        over_budget = current_sum > budget
        scale = torch.ones(N, device=store.weight.device)
        # For over-budget nodes: scale = budget / current (< 1.0)
        mask = over_budget & (current_sum > 1e-8)
        scale[mask] = budget[mask] / current_sum[mask]
...
def main():
    print('=' * 60, flush=True)
    print('  HARD NORMALISATION v2: Ceiling Constraint', flush=True)
    print('  Budget = max, not target. Decay allowed, overshoot blocked.', flush=True)
    ...
    for et, b in budgets.items():
        ...
        print(f'  Ceiling {et.name}: {nonzero} nodes, mean={b[b>0].mean():.4f}', flush=True)
    ...
    for cycle in range(N_CYCLES):
        ...
        for pat in [pa, pb]:
            for s in range(PD):
                ...
                normalise_weights_ceiling(graph, budgets)
        ...
        if (cycle + 1) % 500 == 0:
            ...
            # Check how many nodes are at ceiling vs below
            at_ceiling = 0
            below_ceiling = 0
            for et, budget in budgets.items():
                if not graph.has_edge_type(et): continue
                ...
                nonzero = budget > 1e-8
                at_ceil = ((current[nonzero] - budget[nonzero]).abs() < 0.01 * budget[nonzero]).sum().item()
                below = (current[nonzero] < budget[nonzero] * 0.99).sum().item()
                at_ceiling += at_ceil
                below_ceiling += below

            mm = f'{ratio:.3f}x' + (' **' if ratio > 1.1 else '')
            print(f'  Cyc {cycle+1:5d}: mm={mm} sup={sup:.1f}% ceil={at_ceiling} below={below_ceiling} ({elapsed:.0f}s)', flush=True)
    ...
    torch.save({'all_mm': all_mm, 'errors': errors, 'damping': damping,
               ...
               'hard_normalisation_v2_results.pt')
