...
def apply_normalised_hebbian(graph, la):
    ...
    for et in EdgeType:
        if graph.has_edge_type(et) and et != EdgeType.ELECTRICAL:
            ...
            # Oja's normalisation: divide by mean activity (not squared)
            # Removes quadratic amplification, preserves linear learning
            hebbian = (src * dst) / mean_activity
...
def main():
    print('=' * 60, flush=True)
    print("  OJA'S RULE: Linear normalisation (not quadratic)", flush=True)
    print('=' * 60, flush=True)
    print(f'dw = (pre*post)/mean_activity - decay - penalty', flush=True)
    print(f'Removes quadratic amplification, preserves linear learning', flush=True)
