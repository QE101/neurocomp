"""Theta + Error-Gated Hebbian: only update edges that are wrong.

The direct fix for catastrophic interference. The Hebbian rule only
modifies edges whose DESTINATION NODE has high prediction error.
Edges serving well-predicted nodes are frozen — they can't be undone
by the next pattern's learning.

During pattern A: A-error nodes have high error → their incoming edges update.
                  B-predicted nodes have low error → their edges are PROTECTED.
During pattern B: vice versa.

The two patterns can't interfere because correctly-learned edges
are invisible to the learning rule.
...
LAMBDA_ACT = 3.1
ERROR_THRESHOLD = 2.0  # only update edges whose dst has error above this
...
def error_node_update(ns, inputs, dt=1.0, noise_std=0.005, theta_mod=1.0):
    ...
    # Update error_mean_ema for self-referencing gate (tau=500 steps)
    alpha_ema = dt / 500.0
    ns.error_mean_ema = torch.where(
        exc_mask,
        ns.error_mean_ema * (1 - alpha_ema) + exc_output * alpha_ema,
        ns.error_mean_ema,
    )
    pv_f = pv_mask.float()
...
def apply_hebbian_error_gated(graph, la):
    """Hebbian with SELF-REFERENCING error gate.

    Each node compares its current error to its OWN historical average.
    Doing better than usual → protect edges (don't undo good learning).
    Doing worse than usual → update edges (something needs fixing).

    gate = sigmoid(scale * (current_error - my_error_ema))
    - current > my_average: doing worse than usual → gate open → learn
    - current < my_average: doing better than usual → gate closed → protect

    No global threshold. No parameter to tune. Each node self-determines
    when it's learned enough based on its own history.

    Uses error_mean_ema already tracked in NodeState.
    """
    ...
    for et in EdgeType:
        if graph.has_edge_type(et) and et != EdgeType.ELECTRICAL:
            ...
            # Self-referencing gate: compare dst error to its own history
            dst_ema = ns_.error_mean_ema[store.dst.long()]
            gate = torch.sigmoid(3.0 * (dst - dst_ema))
            # dst > dst_ema: worse than my average → learn (gate ≈ 1)
            # dst < dst_ema: better than my average → protect (gate ≈ 0)
...
def run_step(pat):
    ...
    apply_hebbian_error_gated(graph, LAMBDA_ACT)
    if step % 100 == 0:
        for et in EdgeType:
            if graph.has_edge_type(et) and et != EdgeType.ELECTRICAL:
...
print('Theta + Error-Gated Hebbian: 5000 cycles', flush=True)
print(f'Theta=6Hz amp=0.5 | SELF-REFERENCING gate (each node vs own EMA)', flush=True)
print(f'Doing better than usual = protect edges, worse than usual = update', flush=True)
print(f'{"Cyc":>5} | {"Err":>7} | {"Sup%":>5} | {"Ap_std":>7} | {"GateMn":>7} | {"MM":>8} | {"Time":>5}', flush=True)
print('-' * 60, flush=True)
...
for cycle in range(5000):
    ...
    if (cycle + 1) % 250 == 0:
        ...
        # Mean gate value (how much of the network is being updated)
        gate_mn = torch.sigmoid(3.0 * (ns.output[exc_idx] - ns.error_mean_ema[exc_idx])).mean().item()
        elapsed = time.perf_counter() - t0
        mm = f'{ratio:.3f}x' + (' **' if ratio > 1.1 else '')
        print(f'{cycle+1:5d} | {errors[-1]:.4f} | {sup:.1f}% | {ap:.4f} | {gate_mn:.4f} | '
              f'{mm:>8} | {elapsed:.0f}s', flush=True)
...
if len(all_mm) >= 8:
    ...
    print(f'\nOscillation: first half range={first_range:.3f} second half range={second_range:.3f}', flush=True)
    if second_range < first_range * 0.5:
        print('DAMPING DETECTED', flush=True)
    elif second_range < first_range * 0.9:
        print('PARTIAL DAMPING', flush=True)
    else:
        print('NO DAMPING', flush=True)
