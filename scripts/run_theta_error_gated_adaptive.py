...
LAMBDA_ACT = 3.1
ERROR_THRESHOLD = 0.5  # only update edges whose dst has error above this
...
def apply_hebbian_error_gated(graph, la):
    """Hebbian with error gating: only update edges whose destination has high error.

    In the universal error model, output = |basal - apical|. High output = high error.

    Gate = sigmoid(scale * (dst_output - threshold))
    - dst_output > threshold: gate ≈ 1.0 → full update (this edge needs to improve)
    - dst_output < threshold: gate ≈ 0.0 → frozen (destination is well-predicted, don't touch)

    Smooth sigmoid gate instead of hard threshold to avoid discontinuities.
    """
    ...
    for et in EdgeType:
        if graph.has_edge_type(et) and et != EdgeType.ELECTRICAL:
            ...
            # ADAPTIVE error gate: threshold is the MEDIAN output of excitatory nodes
            # Nodes below median = better-than-average predictions = protect
            # Nodes above median = worse-than-average predictions = update
            exc_median = ns_.output[ns_.type_mask(NodeType.EXCITATORY)].median()
            gate = torch.sigmoid(5.0 * (dst - exc_median))
            # Always protects the bottom 50% and updates the top 50%
            # As the system improves, the median drops, threshold drops, more gets protected
...
print(f'Theta=6Hz amp=0.5 | ADAPTIVE threshold = median excitatory output', flush=True)
print(f'Bottom 50% of nodes protected, top 50% updated', flush=True)
...
for cycle in range(5000):
    ...
    if (cycle + 1) % 250 == 0:
        ...
        exc_med = ns.output[exc_idx].median().item()
        gate_mn = torch.sigmoid(5.0 * (ns.output[exc_idx] - exc_med)).mean().item()
