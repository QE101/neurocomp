...
global_lr = 1.0
ERROR_EMA = None
ERROR_EMA_TAU = 500.0  # smooth over 500 steps = 5 cycles
BRAKE_STRENGTH = 50.0  # how hard to brake when error rises

def apply_hebbian_population_brake(graph, la):
    """Hebbian with population-level brake.

    Track the population mean error. When it's INCREASING (system getting
    worse = collective overshoot), reduce the global learning rate.
    When it's DECREASING (system improving = learning), full speed.

    One number. Population-level. Applied to all edges.
    The moment the collective drift starts pushing error UP, everything freezes.
    """
    global prev_mean_error, global_lr, ERROR_EMA
    ns_ = graph.node_state

    # Compute population mean error (output IS error in universal model)
    exc_mask = ns_.type_mask(NodeType.EXCITATORY)
    current_error = ns_.output[exc_mask].mean().item()

    # Smooth with EMA to avoid step-to-step noise
    if ERROR_EMA is None:
        ERROR_EMA = current_error
    else:
        alpha = 1.0 / ERROR_EMA_TAU
        ERROR_EMA = ERROR_EMA * (1 - alpha) + current_error * alpha

    # Rate of change: is the population getting better or worse?
    if prev_mean_error is not None:
        delta_error = ERROR_EMA - prev_mean_error
        if delta_error > 0:
            # Error INCREASING — overshoot in progress — brake
            global_lr = 1.0 / (1.0 + delta_error * BRAKE_STRENGTH)
        else:
            # Error DECREASING — learning is working — release brake
            global_lr = min(global_lr + 0.01, 1.0)  # slowly release, don't snap
    prev_mean_error = ERROR_EMA
...
def run_step(pat):
    ...
    apply_hebbian_population_brake(graph, LAMBDA_ACT)
    if step % 100 == 0:
        for et in EdgeType:
            if graph.has_edge_type(et) and et != EdgeType.ELECTRICAL:
...
print('Theta + Population brake: 5000 cycles', flush=True)
print('Theta=6Hz amp=0.5 | Global LR drops when population error increases', flush=True)
print(f'{"Cyc":>5} | {"Err":>7} | {"Sup%":>5} | {"Ap_std":>7} | {"GlobLR":>7} | {"ErrEMA":>7} | {"MM":>8} | {"Time":>5}', flush=True)
print('-' * 70, flush=True)
...
for cycle in range(5000):
    ...
    if (cycle + 1) % 250 == 0:
        ...
        print(f'{cycle+1:5d} | {errors[-1]:.4f} | {sup:.1f}% | {ap:.4f} | {global_lr:.4f} | '
              f'{ERROR_EMA:.4f} | {mm:>8} | {elapsed:.0f}s', flush=True)

print(f'\nBest mismatch: {max(all_mm):.3f}x', flush=True)
print(f'Final mismatch: {all_mm[-1]:.3f}x', flush=True)
print(f'Global LR: {global_lr:.4f}, Error EMA: {ERROR_EMA:.4f}', flush=True)
