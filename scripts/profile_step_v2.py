"""Profile step breakdown AFTER fused plasticity + vectorized inh.

Shows the new bottlenecks to target next.
"""
...
def main():
    print("=" * 60)
    print("  POST-FUSION STEP PROFILER")
    print("=" * 60)
    ...
    for i in range(100):
        ...
        ns.apical += (-ns.apical / 20.0 + inputs.apical * (1.0 - torch.sigmoid(inputs.sst_inhibition * 5.0))) * exc_f
    ...
    sections = ['content_compute', 'send_output', 'send_content', 'send_electrical',
                'read_buffer', 'node_exc', 'node_inh', 'noise_ema',
    ...
    for i in range(N_STEPS):
        ...
        content = F.softplus(ns.basal).clamp(max=10.0)
        t1.record()
        times['content_compute'].append((t0, t1))
        ...
        # Node update: inhibitory (vectorized)
        t0 = torch.cuda.Event(enable_timing=True); t1 = torch.cuda.Event(enable_timing=True)
        t0.record()
        inh_input = inputs.basal + inputs.electrical * pv_f
        ns.basal += (-ns.basal / 10.0 + inh_input) * inh_f
    ...
    print(f"\n{'='*60}")
    print(f"  POST-FUSION BREAKDOWN ({N_STEPS} steps, skip {SKIP})")
    print(f"{'='*60}")
    ...
    print(f"  {'overhead':<22} {total - accounted:>10.3f} {(total-accounted)/total*100:>9.1f}%")

    mp_total = results['send_output'] + results['send_content'] + results['send_electrical'] + results['read_buffer'] + results['content_compute']
